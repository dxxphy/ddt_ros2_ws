...
mjModel* LoadModelHeadless(const char* file) {
  char load_error[kErrorLength] = "";
  mjModel* model = nullptr;

  const auto filename_length = std::strlen(file);
  if (filename_length > 4 &&
      !std::strncmp(file + filename_length - 4, ".mjb", 4)) {
    model = mj_loadModel(file, nullptr);
    if (!model) {
      std::printf("could not load binary model\n");
    }
  } else {
    model = mj_loadXML(file, nullptr, load_error, kErrorLength);
    if (!model) {
      std::printf("%s\n", load_error);
    }
  }
...
void PhysicsThread(mj::Simulate* sim, rclcpp::Node::SharedPtr node,
  ...
  double startup_pause_sec = 0.0;
  if (node->has_parameter("startup_pause_sec")) {
    startup_pause_sec = node->get_parameter("startup_pause_sec").as_double();
  }
  if (startup_pause_sec > 0.0) {
    sim->run = 0;
    const auto pause_start = mj::Simulate::Clock::now();
    while (
      rclcpp::ok() && !sim->exitrequest.load() &&
      Seconds(mj::Simulate::Clock::now() - pause_start).count() < startup_pause_sec) {
...
void HeadlessPhysicsLoop(
    rclcpp::Node::SharedPtr node,
    rclcpp::NodeOptions node_options,
    const char* filename,
    const std::vector<std::string>& physics_plugin_names) {
  m = LoadModelHeadless(filename);
  if (!m) {
    RCLCPP_ERROR(node->get_logger(), "Failed to load MuJoCo model in headless mode");
    return;
  }

  d = mj_makeData(m);
  if (!d) {
    RCLCPP_ERROR(node->get_logger(), "Failed to allocate MuJoCo data in headless mode");
    mj_deleteModel(m);
    m = nullptr;
    return;
  }
  ...
  if (!physics_plugin_names.empty()) {
    bool success = true;
    physics_plugin_loader =
      std::make_unique<pluginlib::ClassLoader<mujoco_sim_ros2::MujocoPhysicsPlugin>>(
        "mujoco_sim_ros2", "mujoco_sim_ros2::MujocoPhysicsPlugin");
    try {
      for (const auto& plugin_name : physics_plugin_names) {
        physics_plugins.push_back(physics_plugin_loader->createSharedInstance(plugin_name));
      ...
      RCLCPP_ERROR(node->get_logger(), "The plugin failed to load: %s", ex.what());
      success = false;
    }
    if (success) {
      std::cout << "Successfully loaded " << physics_plugins.size()
  ...
  if (startup_pause_sec > 0.0) {
    rclcpp::sleep_for(std::chrono::duration_cast<std::chrono::nanoseconds>(
      std::chrono::duration<double>(startup_pause_sec)));
  }

  const auto step_period = std::chrono::duration<double>(m->opt.timestep);
  while (rclcpp::ok()) {
...
int main(int argc, char** argv) {
  ...
  node->declare_parameter("startup_pause_sec", 0.0);
  ...
  double startup_pause_sec = node->get_parameter("startup_pause_sec").as_double();
  ...
  std::cout << "startup pause: " << startup_pause_sec << " s" << std::endl;
  std::cout << "========================================" << std::endl;

  if (headless) {
    HeadlessPhysicsLoop(node, cm_node_options, model_file.c_str(), physics_plugins);
    return 0;
  }
