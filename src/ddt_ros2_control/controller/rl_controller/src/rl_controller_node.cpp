...
void RlController::update_control_parameters()
{
  ...
  if (get_node()->has_parameter("joint_pd.target_jpos")) {
    get_node()->get_parameter<std::vector<scalar_t>>("joint_pd.target_jpos", jp_params.target_jpos);
  } else {
    jp_params.target_jpos.clear();
  }
  // RLParameters
  auto get_policy_params = [this](const std::string & policy_name, RLParameters & rl_params) {
    ...
    if (get_node()->has_parameter(policy_name + ".action_filter_alpha")) {
      get_node()->get_parameter<scalar_t>(
        policy_name + ".action_filter_alpha", rl_params.action_filter_alpha);
    }
    if (get_node()->has_parameter(policy_name + ".torque_control")) {
      get_node()->get_parameter<bool>(policy_name + ".torque_control", rl_params.torque_control);
    }
    get_node()->get_parameter<std::vector<scalar_t>>(
    ...
    if (get_node()->has_parameter(policy_name + ".obs_reindex")) {
      get_node()->get_parameter<std::vector<long int>>(
        policy_name + ".obs_reindex", rl_params.obs_reindex);
    }
    if (get_node()->has_parameter(policy_name + ".action_reindex")) {
      get_node()->get_parameter<std::vector<long int>>(
