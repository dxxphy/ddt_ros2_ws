...
: FSMState(data, stateName)
{
  ...
  // }
  inferrer_->loadModel(rl_params_->policy_path, true);
  ...
  control_action_vec_.setZero(rl_params_->num_actions);
  last_raw_control_action_vec_.setZero(rl_params_->num_actions);
...
void FSMState_RL::run()
{
  ...
  for (int i = 0; i < rl_params_->num_actions; i++) {
    tensor_element_t action_scaled = control_action_vec_[i] * rl_params_->action_scales[i];
    const bool is_wheel_joint =
      std::find(_data->params->wheel_indices.begin(), _data->params->wheel_indices.end(), i) !=
      _data->params->wheel_indices.end();
    const tensor_element_t default_angle =
      static_cast<tensor_element_t>(rl_params_->default_joint_angles[i]);
    if (rl_params_->control_type == "P") {
      if (rl_params_->torque_control) {
        // Match IsaacGym wheeled_titatit training:
        // non-wheel: tau = kp * (target - q) - kd * dq
        // wheel: tau = 10 * action_scaled - 0.5 * dof_vel
        ...
        if (is_wheel_joint) {
          ...
          const tensor_element_t joint_pos_target = action_scaled + default_angle;
          _data->low_cmd->tau_cmd(i) =
            rl_params_->joint_kp[i] * (joint_pos_target - pos[i]) - rl_params_->joint_kd[i] * vel[i];
        }
      } else if (is_wheel_joint) {
        _data->low_cmd->kp(i) = 0.0;
        _data->low_cmd->kd(i) = 0.0;
        _data->low_cmd->qd(i) = 0.0;
        _data->low_cmd->qd_dot(i) = 0.0;
...
void FSMState_RL::update_forward()
{
  ...
  while (threadRunning) {
    ...
    if (!stop_update_) {
      ...
      if (inferrer_->getInputNames().size() == 1) {
        const auto & input_shapes = inferrer_->getInputShapes();
        size_t input_size = 1;
        for (const auto dim : input_shapes[0]) {
          input_size *= static_cast<size_t>(dim);
        }
        if (input_size > input_data_2.size()) {
          throw std::runtime_error(
            "[FSMState_RL] Policy history input is larger than configured observation history");
        }
        input_datas.push_back(std::vector<tensor_element_t>(
          input_data_2.end() - static_cast<std::ptrdiff_t>(input_size), input_data_2.end()));
      } else {
      ...
      DVec<tensor_element_t> raw_control_action_vec_ = action_reindex(action_vec_);
      raw_control_action_vec_ = re_sign(raw_control_action_vec_);
      const tensor_element_t new_action_weight =
        static_cast<tensor_element_t>(rl_params_->action_filter_alpha);
      control_action_vec_ =
